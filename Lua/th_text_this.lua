...
function process_this_rules(this_rules, filter_property_func, checkblocked)
    local final_options = {}
    ...
    for i, rules in ipairs(this_rules) do
        ...
        if not is_name_text_this(target) then
            table.insert(target_options, {rule = rule, conds = conds})
        else
...
function do_subrule_this()
    ...
    process_this_rules(this_mod_globals.deferred_rules_with_this, block_filter, false)
    update_raycast_units(true)
    process_this_rules(this_mod_globals.deferred_rules_with_this, other_filter, true)
end