...
function inside(name,x,y,dir_,unitid,leveldata_)
	...
	if (featureindex[name] ~= nil) then
		for i,rule in ipairs(featureindex[name]) do
			...
			if (target == name) and (verb == "has") and (findnoun(object,nlist.short) or (unitreference[object] ~= nil)) and not splice_mod_globals.exclude_from_cut_blocking[unitid] then
					...
					for a,mat in pairs(objectlist) do
						if (a == object) and (object ~= "empty") then
							...
							elseif (string.sub(object, 1, 5) == "group") then
								--[[
								This part should already be handled by rule parsing
