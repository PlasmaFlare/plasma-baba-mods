...
function br_process_branches(branches, br_dir, found_branch_on_last_word, limiter)
    ...
    for _, branch in ipairs(branches) do
        ...
        for variant_num = 1, lhs_totalvariants do
            ...
            -- The content of this "combo" data structure implements something similar to continuously adding 1 to a number of say base 2 (though it could be any base). However in this case, each "digit" of that number has its own
            -- base thats equal to the number of words in that slot.
        ...
        for _, lhs_sentence_data in ipairs(lhs_sentences) do
            for br_word_index, br_word in ipairs(branch.branching_texts) do
                for br_sent_index, br_sentence in ipairs(br_sentences) do
                    ...
                    assert(#br_sentence == #perp_metadata.branching_points_bitfield, tostring(#br_sentence).." "..tostring(#perp_metadata.branching_points_bitfield).." ".. s_display)
                    for i, word in ipairs(br_sentence) do
                        table.insert(final_sentence, word)
                    ...
                    --@TODO(sent id)
                    -- final_sentid = final_sentid..toggle_sent_id(tostring(br_word_index - 1))
